...
if(whichConfig == 1) {
    ...
    await $`yarn --cwd /var/www/chemalot/api/rest`;

    echo(chalk.blue("Running For API App with pm2"));

    await $`pm2 start --name=mock-rest yarn --cwd /var/www/chemalot/api/rest -- start:prod`;
} else {
    echo(chalk.blue("Install packages"));
    await $`yarn --cwd /var/www/chemalot/api/graphql`;

    echo(chalk.blue("Running For API App with pm2"));

    await $`pm2 start --name=mock-graphql yarn --cwd /var/www/chemalot/api/graphql -- start:prod`;
}
