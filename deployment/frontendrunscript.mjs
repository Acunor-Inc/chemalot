...
await $`yarn --cwd /var/www/chemalot/`;
...
if (whichConfig == 1) {
    ...
    await $`pm2 --name shop-rest start yarn --cwd /var/www/chemalot -- run start:shop-rest`;

    echo(chalk.blue("Running For Admin Rest API with pm2"));

    await $`pm2 --name admin-rest start yarn --cwd /var/www/chemalot -- run start:admin-rest`;
} else {
    echo(chalk.blue("Running For Shop GraphQL API with pm2"));

    await $`pm2 --name shop-gql start yarn --cwd /var/www/chemalot -- run start:shop-gql`;

    echo(chalk.blue("Running For Admin GraphQL API with pm2"));

    await $`pm2 --name admin-gql start yarn --cwd /var/www/chemalot -- run start:admin-gql`;
}
