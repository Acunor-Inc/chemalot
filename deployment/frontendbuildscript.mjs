...
await $`scp ${front_end_source_path} ${username}@${ip_address}:/var/www/chemalot`;
echo(chalk.green("Uploaded frontend.zip to server"));

await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "unzip /var/www/chemalot/frontend.zip -d /var/www/chemalot";`;
